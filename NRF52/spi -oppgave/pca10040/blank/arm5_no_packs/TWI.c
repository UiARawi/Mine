...
void SlaveInterruptCB(nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{		
	INT_FROM_SLAVE= true;
}
 ...
 void Reg_config(void)
{
 ...
 void gpiote_init(void){
	...
	nrfx_err = nrfx_gpiote_in_init(GPIO_PIN_4, &switch_config,SlaveInterruptCB);
	
	nrfx_gpiote_in_event_enable(PCAL6408_INT, true);
	nrfx_gpiote_in_event_enable(GPIO_PIN_4, true);
