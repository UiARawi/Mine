...
osThreadDef (SwMon, osPriorityNormal, 1, 0); 
...
void ExtCBInit (void)
{
	...
	nrfx_err = nrfx_gpiote_in_init(PCAL6408_INT_PIN_NR, &switch_config, SMICB);	
		if (nrfx_err != NRFX_SUCCESS)
...
//initialiserer en irq handler med calback fx og setter inn struckten med data
	nrfx_err = nrfx_gpiote_in_init(MESSAGE_FROM_SLAVE, &switch_config, switch_message_from_slave);	
		if (nrfx_err != NRFX_SUCCESS)
            PrintServerPrintf("IRQ handler init failed\n");
	nrfx_gpiote_in_event_enable(MESSAGE_FROM_SLAVE, true);			
		if (nrfx_err != NRFX_SUCCESS)
            PrintServerPrintf("IRQ handler enabledefailed\n");			
}

void switch_message_from_slave (uint32_t pin, nrf_gpiote_polarity_t polarity)
{
...
bool SwitchMonitorInit(void)
{
	...
	/*make a thread*/
	SwitchControl_id = osThreadCreate (osThread(SwMon), NULL);
	return true;
}
...
void SMICB(uint32_t pin, nrf_gpiote_polarity_t polarity)
{	
	//set signal to switchmonitor
		osMessagePut(twi_msg_qid, (uint32_t)pin, 0);	
}

void SwMon(void const *argument)
{
	uint8_t sample_data;
